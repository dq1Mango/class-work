# completely yoinked this from stack github btwa
# https://github.com/hafizmdyasir/Hoshen-Kopelman-Algorithm/blob/master/twodimensions.py

from itertools import product

# from printer import print2DMatrix, printDictionary
...
def hoshenKopelman(grid):
    ...
    for i, j in product(range(len(grid)), range(len(grid[0]))):
        ...
        # XOR is true when only one is true
            ...
            grid[i][j] = addToCluster(
                grid[i - 1][j] if occupiedAbove else grid[i][j - 1]
            )
